...
exports.getTrelloCards = async ({ key, token, board_id }) => {
  const getData = params =>
    axios.get(`https://api.trello.com/1/${params}&key=${key}&token=${token}`);

  const results = [];
  try {
    const { data: lists } = await getData(
      `boards/${board_id}/lists?lists=all&fields=id,name`
    );
